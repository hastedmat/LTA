...
%goal% source and goal taken as additional vertices in the path planning to ease planning of the robot
...
%      waitforbuttonpress; 
%  end
      for mv=2:length(vertex) %iterate through all edges from the node
         ...
         if checkallPath(n(1:2),newPos,map) %if path from n to newPos is collission-free
              ...
              %line([n(2);newPos(2)],[n(1);newPos(1)],'color','r');
               s=s+1;
