...
source=round(tb);
Q=[source 0  heuristic(vertex(1,:),goal) 0+heuristic(vertex(1,:),goal)   -1]; % the processing queue of A* algorihtm, open list
...
%  end
      for mv=2:length(vertex) %iterate through all corners to check for local tangents
         ...
         if checkPath1(n(1:2),newPos,map) %if path from n to newPos is collission-free
...
%for i = 2:NumberTimeStamps
   ...
   % input1 = 'Enter the angle of 1st manipulator = ';
   % d1 = input(input1);
